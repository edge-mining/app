...
const emit = defineEmits<{
  edit: [provider: EnergyLoadHistoryProvider];
  delete: [provider: EnergyLoadHistoryProvider];
}>();
...
const assignedDevices = computed(() => {
  ...
  return props.allDevices.filter(
    (d) => d.energy_load_history_provider_id === props.provider.id
  );
});

const adapterConfig = computed(() => {
  const configs: Record<string, { icon: typeof PhChartLine; styleConfig: CardStyleConfig }> = {
    dummy: {
      icon: PhGear,
      styleConfig: {
  ...
  >
    ...
    <div class="space-y-3">
      <div class="flex items-center gap-2">
        <PhChartLine :size="16" class="text-base-content/50" />
