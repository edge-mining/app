...
import { ClimateMonitorAdapter, type ClimateMonitor } from "../../core/models/climateMonitor";
import { useClimateMonitorStore } from "../../core/stores/climateMonitorStore";
import { useExternalServiceStore } from "../../core/stores/externalServiceStore";
import ClimateMonitorConfigForm from "./ClimateMonitorConfigForm.vue";
...
// Local form state
const formData = ref<ClimateMonitor>({
  name: "",
...
async function updateCompatibleExternalServices(adapterType: string) {
  ...
  try {
    const resp = await climateMonitorStore.externalServices(adapterType);
...
watch(
  ...
  (isOpen) => {
    if (isOpen) {
      if (props.climateMonitor) {
        formData.value = {
          ...props.climateMonitor,
          config: props.climateMonitor.config
            ? { ...props.climateMonitor.config }
            : {},
          external_service_id: props.climateMonitor.external_service_id || "",
        };
        if (props.climateMonitor.adapter_type) {
          updateCompatibleExternalServices(props.climateMonitor.adapter_type);
        }
      } else {
        formData.value = {
          name: "",
          adapter_type: climateMonitorStore.adapterTypes[0] || ClimateMonitorAdapter.HOME_ASSISTANT_API,
          config: {},
          external_service_id: "",
        };
...
function cleanClimateMonitor(monitor: ClimateMonitor): ClimateMonitor {
...
function handleSave() {
  if (isFormValid.value) {
    ...
    const cleanedData = cleanClimateMonitor(rawData);
      ...
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
          ...
          <h3 class="text-xl font-bold">
            {{ isEdit ? "Edit Climate Monitor" : "Add Climate Monitor" }}
          </h3>
        </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="form-control">
              ...
              >
                ...
                <option
                  v-for="adapterType in climateMonitorStore.adapterTypes"
                  :key="adapterType"
                  :value="adapterType"
                >
